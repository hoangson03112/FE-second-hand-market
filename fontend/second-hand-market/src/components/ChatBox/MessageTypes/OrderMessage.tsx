...
const OrderMessage: React.FC<OrderMessageProps> = ({ order }) => {
  ...
  useEffect(() => {
    const fetchTotalAmount = async () => {
      try {
        const result = await orderService.getTotalAmountOfOrder(order.id);
  ...
  const handleViewOrder = () => {
    if (orderId) {
      navigate(`/eco-market/seller/order-details/${orderId}`);
    } else {

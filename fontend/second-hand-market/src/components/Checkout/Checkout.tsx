...
const Checkout = () => {
  ...
  const shippingFeeValue = getTotalShippingFee() || 0;
  const finalAmount = Number(totalAmount) + Number(shippingFeeValue); 
  ...
  const handleShippingMethodChange = (sellerId: string, method: any) => {
  ...
  return (
    ...
    >
          ...
          {hasShipCodOrders() && (
            <Grid item xs={12}>
              <PaymentMethodSection
                ...
                shippingFee={Number(getTotalShippingFee() || 0)}
      ...
      <AddressModal
        ...
        onToggleNewAddressForm={(show: boolean) => setShowNewAddressForm(show)}
        ...
        onAddressAdded={(address: any) => handleSelectAddress(address)}
      />
    </Box>
  );
};

...
const CheckoutFooter = ({ onPlaceOrder, isLoading = false }) => {
  return (
    <Paper 
      elevation={1} 
      sx={{ 
    ...
    >
      <Box sx={{ display: { xs: 'block', md: 'flex' }, alignItems: 'center', justifyContent: 'space-between', gap: 3 }}>
        {/* Security Information */}
        <Box sx={{ flex: 1, mb: { xs: 2, md: 0 } }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            <ShieldOutlined sx={{ mr: 1, color: 'text.primary', fontSize: 20 }} />
        ...
        {/* Order Action */}
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: { xs: 'stretch', md: 'center' }, gap: 1 }}>
          <Button
            variant="contained"
            size="large"

import ApiService from "../services/ApiService";
import storage from "../utils/storage";

class SubCategoryContext {
  async getSubCategory(SubcategoryID) {
    try {
      const response = await ApiService.get("/categories/sub", {
        params: { SubcategoryID },
      });

      return response;
    } catch (error) {
      console.error("Error fetching data:", error);
      return;
    }
  }
  async addSubcategory(subcategory, parentCategoryId) {
    try {
      const response = await ApiService.post(`/categories/sub/${parentCategoryId}`, {
        name: subcategory.name,
        status: subcategory.status,
      });
      return response;
    } catch (error) {
      console.error("Error adding data:", error);
      return;
    }
  }
  async updateSubcategory(subcategory, parentCategoryId) {
    try {
      const response = await ApiService.put("/categories/sub/update", {
        subcategory,
        parentCategoryId,
      });
      return response;
    } catch (error) {
      console.error("Error updating data:", error);
      return;
    }
  }
  async deleteSubcategory(subcategoryID, categoryId) {
    try {
      const response = await ApiService.delete(
        `/categories/${categoryId}/sub/${subcategoryID}`
      );
      return response;
    } catch (error) {
      console.error("Error deleting data:", error);
      return;
    }
  }
}
// eslint-disable-next-line import/no-anonymous-default-export
export default new SubCategoryContext();


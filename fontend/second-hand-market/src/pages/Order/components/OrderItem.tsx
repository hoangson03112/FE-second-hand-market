...
const OrderItem = ({ order, setOrders }) => {
  ...
  useEffect(() => {
    const fetchReview = async () => {
      try {
        const res = await ApiService.get(`/seller-reviews/by-order/${order._id}`);
        if (res.data.review) {
          setHasReview(true);
  ...
  const handleCancelOrder = async (orderId, reason, status, bankInfo) => {
    try {
      if (bankInfo) {
        await ApiService.post(`/bank-info`, {
          orderId,
  ...
  const handlePayment = async () => {
    try {
      ...
      const response = await ApiService.post("/payments/create-payment-link", {
        orderId: order._id,
